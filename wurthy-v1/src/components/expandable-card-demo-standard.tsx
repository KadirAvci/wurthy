...
export default function ExpandableCardDemo() {
  ...
  return (
    <>
                ...
                <div className="tw:flex tw:justify-between tw:items-start tw:p-4">
                  <div className="tw:">
                    <motion.h3
                      ...
                      className="tw:font-bold tw:text-neutral-700 tw:dark:text-neutral-200"
                    >
                      {active.title}
                    </motion.h3>
                    <motion.p
      ...
      <ul className="tw:max-w-2xl tw:mx-auto tw:w-full tw:gap-4">
        {cards.map((card) => (
          <motion.div
            layoutId={`card-${card.title}-${id}`}
            key={`card-${card.title}-${id}`}
            onClick={() => setActive(card)}
            className="tw:p-4 tw:flex tw:flex-col tw:md:flex-row tw:justify-between tw:items-center tw:hover:bg-neutral-50 tw:dark:hover:bg-neutral-800 tw:rounded-xl tw:cursor-pointer"
          >
            <div className="tw:flex tw:gap-4 tw:flex-col tw:md:flex-row tw:">
              <motion.div layoutId={`image-${card.title}-${id}`}>
                <img
                  ...
                  src={card.src}
                  alt={card.title}
                  className="tw:h-40 tw:w-40 tw:md:h-14 tw:md:w-14 tw:rounded-lg tw:object-cover tw:object-top"
                />
              </motion.div>
              <div className="tw:">
                <motion.h3
                  layoutId={`title-${card.title}-${id}`}
                  className="tw:font-medium tw:text-neutral-800 tw:dark:text-neutral-200 tw:text-center tw:md:text-left"
                >
                  {card.title}
                </motion.h3>
                <motion.p
                  layoutId={`description-${card.description}-${id}`}
                  className="tw:text-neutral-600 tw:dark:text-neutral-400 tw:text-center tw:md:text-left"
                >
                  {card.description}
                </motion.p>
              </div>
            ...
            <motion.button
              ...
              className="tw:px-4 tw:py-2 tw:text-sm tw:rounded-full tw:font-bold tw:bg-gray-100 tw:hover:bg-green-500 tw:hover:text-white tw:text-black tw:mt-4 tw:md:mt-0"
            >
              {card.ctaText}
            </motion.button>
          </motion.div>
